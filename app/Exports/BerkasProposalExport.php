...
class BerkasProposalExport implements FromCollection,  WithHeadings, WithMapping
{
    /**
    ...
    {
        return BerkasSidangProposal::with('pengajuanDospem')
            ->whereHas('pengajuanDospem.mahasiswa', function ($query) {
                $query->where('prodi', $this->prodi);
            })->get();

        if ($this->id) {
            return BerkasSidangProposal::whereHas('pengajuanDospem', function ($query) {
                $query->where('dp1_id', $this->id)->orWhere('dp2_id', $this->id)->orWhereHas('berkasProposal.jadwalProposals.jadwalSidang', function ($query) {
    ...
    public function map($row): array
    {
        return [
            ...
            'Dosen Pembimbing 2' => (string) $row->pengajuanDospem->dospem2->nama_dosen ?? '-',
