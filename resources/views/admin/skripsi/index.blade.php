...
<div class="row justify-content-center">
    <div class="col-md-12">
        <h1 class="mt-4 mb-4">Pendaftaran Sidang Skripsi {{$prodi}}</h1>
        <div class="row mb-2 mt-4">
            <div class="col-md-6">
                <a href="{{ route('berkas-skripsi.export', ['prodi' => $prodi]) }}" id="exportButton" class="btn btn-outline-secondary">Export</a>
            </div>

            <div class="col-md-3 offset-md-3 text-md-end">
                <form action="{{ route('berkas-skripsi.index') }}" method="GET">
                    <div class="input-group mb-3">
                        ...
                        @if ($search)
                        <a href="{{ route('berkas-skripsi.index', ['prodi'=>$prodi]) }}" class="btn btn-outline-secondary">Clear</a>
                        @endif
                    </div>
                </form>
            </div>
            ...
            <div class="table-container" style="overflow-x: auto;">
                <table class="table table-bordered table-hover">
                    ...
                    <tr>
                        ...
                        <th style="white-space: nowrap;">Scan KTP, KK, Akta Lahir</th>
                        <th style="white-space: nowrap;">Pas Foto Ijazah</th>
                        <th style="white-space: nowrap;">Buku Bimbingan</th>
                        <th style="white-space: nowrap;">Hasil Turnitin</th>
                        <th style="white-space: nowrap;">KHS</th>
                        <th style="white-space: nowrap;">KST</th>
                        <th style="white-space: nowrap;">Print Out Pembayaran UKT</th>
                        <th style="white-space: nowrap;">File Skripsi</th>
                        <th style="white-space: nowrap;">Video Presentasi</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ Storage::url($d->ktp_kk_akta) }}" target="_blank" style="text-decoration: none;">View</a>
                        </td>
                        <td>
                            <a href="{{ Storage::url($d->pas_foto) }}" target="_blank" style="text-decoration: none;">View</a>
                        </td>
                        <td>
                            <a href="{{ Storage::url($d->buku_bimbingan) }}" target="_blank" style="text-decoration: none;">View</a>
                        </td>
                        <td>
                            <a href="{{ Storage::url($d->turnitin) }}" target="_blank" style="text-decoration: none;">View</a>
                        </td>
                        <td>
                        ...
                        <td>
                            <a href="{{ Storage::url($d->ukt) }}" target="_blank" style="text-decoration: none;">View</a>
                        </td>
                        <td>
                        ...
                        <td style="white-space: nowrap;">
                            <button class="btn btn-sm btn-primary open-modal"
                                data-nama_mahasiswa="{{ $d->pengajuanDospem->mahasiswa->nama_mahasiswa }}"
                                data-nim="{{ $d->pengajuanDospem->mahasiswa->nim }}"
                                data-id="{{$d->id}}"
                                {{$d->status !== 'Pending' ? 'disabled' : ''}}>Edit Status
                            </button>
                        </td>
                        <td>{{ $d->keterangan }}</td>
                    </tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Approval Pendaftaran Sidang Skripsi</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>

            <form action="{{ route('berkas-skripsi.update-status') }}" method="POST">
                @csrf
